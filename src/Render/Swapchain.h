namespace legit
{
  ...
  public:
    ...
    std::vector<const ImageView *> GetImageViews()
    {
      std::vector<const ImageView *> resImageViews;
  ...
  private:
    Swapchain(vk::Instance instance, vk::PhysicalDevice physicalDevice, vk::Device logicalDevice, WindowDesc windowDesc, QueueFamilyIndices queueFamilyIndices)
    {
      ...
      this->format = FindSwapchainSurfaceFormat(surfaceDetails.formats);
      this->presentMode = FindSwapchainPresentMode(surfaceDetails.presentModes);
      ...
      {
        Image newbie;
        newbie.image = swapchainImages[imageIndex];
        newbie.imageView = std::unique_ptr<ImageView>(new ImageView(logicalDevice, swapchainImages[imageIndex], vk::ImageViewType::e2D, this->format.format, 0, 1));
